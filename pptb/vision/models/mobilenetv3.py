...
class SqueezeExcitation(nn.Layer):
    def __init__(self, channels, reduction=4):
        ...
        self.conv1 = nn.Conv2D(
            in_channels=channels, out_channels=channels // reduction, kernel_size=1, stride=1, padding=0
        )
        self.relu = nn.ReLU()
        self.conv2 = nn.Conv2D(
            in_channels=channels // reduction, out_channels=channels, kernel_size=1, stride=1, padding=0
        )
...
class ConvBNLayer(nn.Layer):
    def __init__(self, in_channels, out_channels, filter_size, stride, padding, num_groups=1, act=None):
        super().__init__()
...
class InvertedResidual(nn.Layer):
    def __init__(self, in_channels, expanded_channels, out_channels, filter_size, stride, use_se, act=None):
        super().__init__()
        self.if_shortcut = stride == 1 and in_channels == out_channels
        self.if_se = use_se

        self.expand_conv = ConvBNLayer(
            in_channels=in_channels, out_channels=expanded_channels, filter_size=1, stride=1, padding=0, act=act
        )
        ...
        self.linear_conv = ConvBNLayer(
            in_channels=expanded_channels, out_channels=out_channels, filter_size=1, stride=1, padding=0, act=None
        )
...
class InvertedResidualConfig:
    def __init__(self, in_channels, kernel, expanded_channels, out_channels, use_se, activation, stride):
...
class MobileNetV3(nn.Layer):
    ...
    def __init__(self, config, scale=1.0, last_channel=1280, num_classes=1000, with_pool=True):
        super().__init__()
        ...
        self.blocks = nn.Sequential(
            *[
                InvertedResidual(
                    ...
                    expanded_channels=_make_divisible(self.scale * cfg.expanded_channels),
        ...
        self.last_second_conv = ConvBNLayer(
            ...
            out_channels=_make_divisible(self.scale * self.last_expanded_channels),
            filter_size=1,
            stride=1,
            padding=0,
            num_groups=1,
        ...
        if num_classes > 0:
            self.last_conv = nn.Conv2D(
                in_channels=_make_divisible(self.scale * self.last_expanded_channels),
...
class MobileNetV3Small(MobileNetV3):
    ...
    Examples:
        .. code-block:: python
            ...
            # build model
            mobilenet_v3_small_x1_0 = MobileNetV3Small(scale=1.0)
...
class MobileNetV3Large(MobileNetV3):
    ...
    Examples:
        .. code-block:: python
            ...
            # build model
            mobilenet_v3_large_x1_0 = MobileNetV3Large(scale=1.0)
